import type { Faq } from "./types";
import { company } from "./company";
import { publishedZones } from "./zones";

export const globalFaqs: Faq[] = [
  {
    question: "Intervenez-vous la nuit, le week-end et les jours fériés ?",
    answer:
      "Oui. DépannageScooter est disponible 24h/24 et 7j/7 en Île-de-France. Les interventions hors horaires de jour font l'objet d'une majoration indiquée sur la page tarifs, confirmée par téléphone avant déplacement.",
  },
  {
    question: "Combien de temps faut-il pour qu'un dépanneur arrive ?",
    answer:
      "En journée, comptez en moyenne 25 à 40 minutes selon la zone et le trafic. Le délai exact vous est annoncé lors de votre appel, avant engagement. La nuit et le week-end, le délai peut être légèrement plus long.",
  },
  {
    question: "Quels véhicules prenez-vous en charge ?",
    answer:
      "Scooters 50 à 125 cm³, maxi-scooters, motos, trois-roues et voitures (citadines, berlines, SUV, utilitaires légers). Pour une batterie voiture à plat, consultez la page dépannage batterie voiture. Indiquez marque et modèle lors de l'appel.",
  },
  {
    question: "Intervenez-vous pour une batterie de voiture à plat ?",
    answer:
      "Oui. Démarrage au booster ou remplacement de batterie voiture à domicile en Île-de-France, 24h/24. Test batterie et alternateur sur place. Devis ferme au téléphone avant déplacement.",
  },
  {
    question: "Comment se paie l'intervention ?",
    answer:
      "Le règlement s'effectue sur place par carte bancaire ou espèces, une fois l'intervention terminée. Un devis ferme vous est communiqué par téléphone avant le départ du dépanneur. Pas de surprise à l'arrivée.",
  },
  {
    question: "Mon assurance peut-elle prendre en charge le dépannage ?",
    answer:
      "De nombreux contrats moto incluent une assistance 0 km ou un remorquage ; les contrats auto proposent souvent la même garantie (dépannage sur place, batterie, remorquage). Consultez votre attestation ou appelez l'assistance de votre assureur avant de payer. Nous fournissons une facture détaillée pour votre dossier.",
  },
  {
    question: "Intervenez-vous sur autoroute ou périphérique ?",
    answer:
      "Sur voie rapide, la sécurité impose parfois l'intervention des services autoroutiers ou du 112 selon l'emplacement. Appelez-nous : nous vous indiquons la procédure adaptée et organisons la prise en charge dès que l'accès est possible.",
  },
  {
    question: "Puis-je choisir la destination en cas de remorquage ?",
    answer:
      "Oui : domicile, garage, concession, fourrière ou expert d'assurance. Précisez l'adresse exacte lors de l'appel. Le tarif dépend de la distance entre le lieu de panne et la destination.",
  },
  {
    question: "Que faire en cas de crevaison sans roue de secours ?",
    answer:
      "Ne roulez pas sur le pneu crevé. Appelez-nous avec votre localisation précise. Nous réparons sur place si la crevaison est réparable, sinon nous organisons un remorquage vers un professionnel.",
  },
  {
    question: "Proposez-vous un dépannage batterie à domicile ?",
    answer:
      "Oui, sur scooter, moto et voiture. Nous testons la batterie et le circuit de charge sur place. Remplacement si nécessaire avec une référence adaptée. Un booster suffit parfois pour repartir — voir les pages batterie deux-roues et batterie voiture.",
  },
  {
    question: "Intervenez-vous dans les parkings souterrains ?",
    answer:
      "Oui, sous réserve d'accès (hauteur, badge, rampe). Signalez un parking souterrain ou un box lors de l'appel : un supplément pénibilité peut s'appliquer selon la complexité d'accès.",
  },
  {
    question: "Comment obtenir un tarif avant l'intervention ?",
    answer:
      `Appelez le ${company.phoneDisplay} ou contactez-nous via WhatsApp. Nous établissons un devis ferme en fonction du type de panne, de votre adresse et de l'horaire. Aucune intervention sans accord préalable.`,
  },
  {
    question: "Enlèvez-vous les épaves ou scooters abandonnés ?",
    answer:
      "Nous assurons le remorquage de deux-roues et de voitures en panne immobilisés. Pour un véhicule abandonné sur voie publique, les démarches passent souvent par la mairie ou la fourrière. Contactez-nous pour orienter votre situation.",
  },
  {
    question: "Quelle est la zone couverte exactement ?",
    answer:
      `Paris intra-muros et l'ensemble de l'Île-de-France (77, 78, 91, 92, 93, 94, 95) : ${publishedZones.length} communes couvertes. Consultez la page zones d'intervention pour la liste complète. Hors zone : devis sur demande.`,
  },
  {
    question: "Puis-je suivre l'arrivée du dépanneur ?",
    answer:
      "Lors de l'appel, nous vous confirmons le délai estimé. Le dépanneur peut vous joindre en route. Restez joignable et visible à l'adresse indiquée, feux de détresse activés si vous êtes sur la chaussée.",
  },
  {
    question: "Que faire si ma clé est cassée ou perdue ?",
    answer:
      "Ne forcez pas le contact ou le Neiman. Selon le modèle, nous pouvons ouvrir ou transporter le véhicule. Décrivez la situation par téléphone : la solution dépend du type de verrouillage et de l'antivol.",
  },
  {
    question: "Remorquez-vous une moto après accident ?",
    answer:
      "Oui. Nous prenons en charge les motos et scooters accidentés ou chutés, avec prise de photos si nécessaire pour l'assurance. Ne déplacez pas le véhicule si les forces de l'ordre n'ont pas libéré les lieux.",
  },
  {
    question: "Faut-il préparer des papiers pour l'intervention ?",
    answer:
      "Ayez votre pièce d'identité et la carte grise du véhicule. En cas de remorquage vers un garage, certains professionnels demandent ces documents. Pour un dossier assurance, conservez la facture d'intervention.",
  },
  {
    question: "Proposez-vous la livraison d'essence ?",
    answer:
      "Oui, en cas de panne sèche nous apportons le carburant adapté (SP95/SP98 ou mélange 2 temps selon le véhicule). Vérifiez que la panne vient bien du réservoir vide et non d'un autre défaut.",
  },
  {
    question: "Combien coûte un remorquage en Île-de-France ?",
    answer:
      "Le tarif dépend de la distance entre le lieu de panne et la destination : paliers de 0 à 5 km, 5 à 10 km, 10 à 15 km et 15 à 20 km, puis sur devis au-delà. Consultez la page tarifs ou appelez pour un devis immédiat avant départ.",
  },
  {
    question: "Intervenez-vous sur les trottinettes électriques ?",
    answer:
      "Non, notre activité couvre les scooters et motos thermiques ou électriques homologués route (L1-L3). Les trottinettes non immatriculées ne font pas partie de notre périmètre.",
  },
  {
    question: "Que faire si mon scooter est volé ou vandalisé ?",
    answer:
      "Déposez plainte et contactez votre assureur. Si le véhicule est retrouvé immobilisé et doit être déplacé, nous pouvons assurer le remorquage vers un lieu sécurisé sur devis.",
  },
  {
    question: "Puis-je annuler après avoir appelé ?",
    answer:
      "Oui, tant que le dépanneur n'est pas en route. Prévenez-nous immédiatement. Si le déplacement a commencé, des frais de déplacement peuvent s'appliquer selon ce qui a été annoncé au téléphone.",
  },
  {
    question: "Travaillez-vous avec les garages et concessions ?",
    answer:
      "Oui. Nous livrons régulièrement des deux-roues chez des réparateurs partenaires en Île-de-France. Indiquez le nom et l'adresse du professionnel lors de l'appel pour organiser le transfert.",
  },
  {
    question: "Comment signaler une panne sur un lieu de livraison (Uber Eats, etc.) ?",
    answer:
      "Donnez l'adresse exacte du point de livraison ou du restaurant. Les zones denses parisiennes (Marais, Opéra, Châtelet) sont fréquemment couvertes. Nous adaptons le point de rendez-vous si l'accès est restreint.",
  },
  {
    question: "Proposez-vous un contrat pour les flottes de livreurs ?",
    answer:
      "Les demandes professionnelles (flottes, loueurs, restaurateurs) se traitent au cas par cas. Contactez-nous par email avec le volume estimé et les zones concernées pour une proposition adaptée.",
  },
];
